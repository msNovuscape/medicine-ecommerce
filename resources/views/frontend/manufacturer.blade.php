@extends('frontend.master')
    ...
    <section class="ps-section--featured">
                                        ...
                                        <div class="ps-product ps-product--standard">
                                            <div class="ps-product__thumbnail"><a class="ps-product__image"
                                                ...
                                                <div class="ps-product__actions">
                                                    <div class="ps-product__item" title="Wishlist"><a href="{{route('wishlist.add',['product_id' => $medicine->id])}}"><i
                                            ...
                                            <div class="ps-product__content">
                                                ...
                                                <div class="ps-product__actions ps-product__group-mobile">
                                                    ...
                                                    href="javascript:add_to_cart({{$medicine->id}})" id="{{$medicine->id}}" route="{{route('frontend.add_to_cart')}}">Add
                                                            to cart</a></div>
                                                    <div class="ps-product__item cart" title="Add to cart"><a href="javascript:add_to_cart({{$medicine->id}})" id="{{$medicine->id}}" route="{{route('frontend.add_to_cart')}}"><i
                                                                class="fa fa-shopping-basket"></i></a></div>
                                                    <div class="ps-product__item" title="Wishlist"><a
                                                        href="{{route('wishlist.add',['product_id' => $medicine->id])}}"><i class="fa fa-heart-o"></i></a>
                                                    </div>
                    ...
                    <div class="ps-pagination mb-50">
                        <ul class="pagination">
                            <li class="{{ ($medicines->currentPage() == 1) ? 'disabled' : '' }}">
                                <a href="{{ $medicines->url(1) }}"><i class="fa fa-angle-double-left"></i></a>
                            </li>
                            @for ($i = 1; $i <= $medicines->lastPage(); $i++)
                                <li class="{{ ($medicines->currentPage() == $i) ? ' active' : '' }}">
                                    <a href="{{ $medicines->url($i) }}">{{ $i }}</a>
                                </li>
                            @endfor
                            <li class="{{ ($medicines->currentPage() == $medicines->lastPage()) ? 'disabled' : '' }}" >
                                <a href="{{ $medicines->url($medicines->currentPage()+1) }}"><i class="fa fa-angle-double-right"></i></a>
                            </li>
                        </ul>
                    </div>    
                    @endif 
