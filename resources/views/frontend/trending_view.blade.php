    ...
    @section('content')
            <div class = "container">
                    <section class="ps-section--featured">
                        <h3 class="ps-section__title">Trending Products</h3>
                        <div class="ps-section__content">
                                        ...
                                        <div class="ps-product ps-product--standard">
                                            <div class="ps-product__thumbnail"><a class="ps-product__image"
                                                ...
                                                <div class="ps-product__actions">
                                                    <div class="ps-product__item" title="Wishlist"><a href="{{route('wishlist.add',['product_id' => $trending_medicine->id])}}"><i
                                            ...
                                            <div class="ps-product__content">
                                                ...
                                                <div class="ps-product__actions ps-product__group-mobile">
                                                    ...
                                                    href="javascript:add_to_cart({{$trending_medicine->id}})" id="{{$trending_medicine->id}}" route="{{route('frontend.add_to_cart')}}">Add
                                                            to cart</a></div>
                                                    <div class="ps-product__item cart" title="Add to cart"><a href="javascript:add_to_cart({{$trending_medicine->id}})" id="{{$trending_medicine->id}}" route="{{route('frontend.add_to_cart')}}"><i
                                                                class="fa fa-shopping-basket"></i></a></div>
                                                    <div class="ps-product__item" title="Wishlist"><a
                                                        href="{{route('wishlist.add',['product_id' => $trending_medicine->id])}}"><i class="fa fa-heart-o"></i></a>
                                                    </div>
                            ...
                            @if ($trending_medicines->lastPage() > 1)
                    <div class="ps-pagination mb-50">
                        <ul class="pagination">
                            <li class="{{ ($trending_medicines->currentPage() == 1) ? 'disabled' : '' }}">
                                <a href="{{ $trending_medicines->url(1) }}"><i class="fa fa-angle-double-left"></i></a>
                            </li>
                            @for ($i = 1; $i <= $trending_medicines->lastPage(); $i++)
                                <li class="{{ ($trending_medicines->currentPage() == $i) ? ' active' : '' }}">
                                    <a href="{{ $trending_medicines->url($i) }}">{{ $i }}</a>
                                </li>
                            @endfor
                            <li class="{{ ($trending_medicines->currentPage() == $trending_medicines->lastPage()) ? 'disabled' : '' }}" >
                                <a href="{{ $trending_medicines->url($trending_medicines->currentPage()+1) }}"><i class="fa fa-angle-double-right"></i></a>
                            </li>
                        </ul>
                    </div>    
                    @endif
